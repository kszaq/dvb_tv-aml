...
static int r848_set_params(struct dvb_frontend *fe)
{
	...
	if (ret) {
		dev_info(&priv->i2c->dev, "tuner lock failed!\n");
		goto exit;
	}
	for (i = 0; i < 5; i++) {
		...
		struct r848_config *cfg, struct i2c_adapter *i2c)
{
	...
	if (priv == NULL) {
		dev_info(&priv->i2c->dev, "attach failed\n");
		return NULL;
	}
	...
	dev_info(&priv->i2c->dev, "Rafael R848 successfully attached\n");
...
EXPORT_SYMBOL_GPL(r848x_attach);
